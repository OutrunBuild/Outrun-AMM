...
contract OutrunAMMPairOnBlast is IOutrunAMMPairOnBlast, OutrunAMMERC20, ReentrancyGuard, BlastGovernorableInit {
    ...
    address public constant WETH = 0x4200000000000000000000000000000000000023;          // OutrunTODO update when mainnet
    address public constant USDB = 0x4200000000000000000000000000000000000022;          // OutrunTODO update when mainnet
    address public constant SY_BETH = 0xC496056C8eE317167fD6aCb7bc6ff8772173f277;       // OutrunTODO update when deploy
    address public constant SY_USDB = 0x45fC619E82306c9d9d6219134a0Ff9202F8c87D9;       // OutrunTODO update when deploy
    ...
    function initialize(
        ...
        __BlastGovernorable_init(_blastGovernor);
        factory = msg.sender;
        MEVGuard = _MEVGuard;
    ...
    function clearBETHNativeYield(address maker) external {
        require(msg.sender == yieldVault, PermissionDenied());
    ...
    function _calcNewYieldIndex(
        ...
        if (yieldAmount > 0) {
            ...
            uint256 syAmount = IStandardizedYield(SY).deposit(yieldVault, nativeYieldToken, yieldAmount, 0);
